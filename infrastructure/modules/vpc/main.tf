# VPC Module - Private Only with VPC Endpoints
...
# Route Table for Public Subnets (ALB only)
...
resource "aws_vpc_endpoint" "securityhub" {
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-securityhub-endpoint"
  })
}

# Inspector Interface Endpoint
resource "aws_vpc_endpoint" "inspector" {
  vpc_id              = aws_vpc.main.id
  ...
  tags = merge(var.common_tags, {
    Name = "${var.name_prefix}-inspector-endpoint"
  })
}

# STS Interface Endpoint (for IAM roles)
